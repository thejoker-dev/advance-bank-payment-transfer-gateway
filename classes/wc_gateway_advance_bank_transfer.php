...
class ABPT_Gateway_Advance_Bank_Payment_Offline extends WC_Payment_Gateway
{
    ...
    public function __construct()
    {
        ...
        $this->method_title       = __('Transferencia Electrónica Avanzada o Deposito Bancario', $this->domain);
        $this->method_description = __('Acepte Transferencia electrónica o Deposito Bancario', $this->domain);
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields = apply_filters('wc_offline_form_fields', array(
            ...
            'enabled' => array(
                'title'   => __('Activar/Desactivar', $this->domain),
                'type'    => 'checkbox',
                'label'   => __('Activar metodo de pago', $this->domain),
                'default' => 'yes'
            ),
            ...
            'title' => array(
                ...
                'type'        => 'text',
                'description' => __('Esto controla el título del método de pago que el cliente ve durante el pago.', $this->domain),
                'default'     => __('Transferencia Electrónica', $this->domain),
                'desc_tip'    => true,
            ),
            ...
            'description' => array(
                ...
                'type'        => 'textarea',
                'description' => __('Descripción del método de pago que el cliente verá en su pago.', $this->domain),
                'default'     => __('Realiza tu pago directamente a nuestra cuenta bancaria mediante transferencia electrónica. Tu pedido no será enviado hasta que el importe haya sido recibido y verificado en nuestra cuenta bancaria.', $this->domain),
                'desc_tip'    => true,
            ),
            ...
            'instructions' => array(
                ...
                'type'        => 'textarea',
                'description' => __('Breves pasos a seguir', $this->domain),
                'default'     => 'Realiza tu pago directamente a nuestra cuenta bancaria mediante transferencia electrónica. Tu pedido no será enviado hasta que el importe haya sido recibido y verificado en nuestra cuenta bancaria.',
                'desc_tip'    => true,
            ),
	...
	public function generate_account_details_html() {
		...
		$sortcode = isset( $locale[ $country ]['sortcode']['label'] ) ? $locale[ $country ]['sortcode']['label'] : __( 'Tipo de Cuenta', 'wc-gateway-offline' );
		?>
		<tr valign="top">
			<th scope="row" class="titledesc"><?php esc_html_e( 'Detalle de Cuentas:', 'wc-gateway-offline' ); ?></th>
				...
				<div class="wc_input_table_wrapper">
					<table class="widefat wc_input_table sortable" cellspacing="0">
						<thead>
							<tr>
								<th class="sort">&nbsp;</th>
								<th><?php esc_html_e( 'Nombre de la Cuenta', 'wc-gateway-offline' ); ?></th>
								<th><?php esc_html_e( 'Número de Cuenta', 'wc-gateway-offline' ); ?></th>
								<th><?php esc_html_e( 'Banco', 'wc-gateway-offline' ); ?></th>
								<th><?php echo esc_html( $sortcode ); ?></th>
								<th><?php esc_html_e( 'Titular de la Cuenta', 'wc-gateway-offline' ); ?></th>
								<th><?php esc_html_e( 'Correo Electrónico', 'wc-gateway-offline' ); ?></th>
							</tr>
						</thead>
						...
						<tfoot>
							<tr>
								<th colspan="7"><a href="#" class="add button"><?php esc_html_e( '+ Añadir Cuenta', 'wc-gateway-offline' ); ?></a> <a href="#" class="remove_rows button"><?php esc_html_e( 'Eliminar las siguientes cuenta(s)', 'wc-gateway-offline' ); ?></a></th>
							</tr>
						</tfoot>
					</table>
				</div>
	...
	public function payment_fields(){
		...
		<div id="custom_input">
			<p class="form-row form-row-wide">
				<label for="bank_payment_receipt" class=""><?php _e('Por favor subir el comprobante de Transferencia Electrónica o Comprobante de Deposito Bancario', $this->domain); ?></label>
				...
				$(".bank_payment_receipt").change( function() {
					...
					$.ajax({
						...
						success: function(response){
							if(response=='0'){
								alert('Archivo subido invalido. Debe ser una imagen.');
								$('.attach_id').val('');
							}else{
	...
	public function validate_fields(){
		if(!isset($_POST['attach_id']) || empty( $_POST['attach_id']) ) {
			wc_add_notice(__('<strong>El comprobante de Transferencia Electrónica o comprobante de Deposito Bancario</strong> es obligatorio.'), 'error');
			return false;
		}
		return true;
	}
    ...
    public function process_payment($order_id)
    {
        ...
        $order->update_status('on-hold', __('En espera de la Transferencia/Deposito', $this->domain));
...
function abpt_custom_checkout_field_display_admin_order_meta($order){
    ...
    echo '<p><strong>'.__( 'Comprobante de Transferencia Electrónica/comprobante de Deposito Bancario' ).':</strong> <a href="'.$src.'"><img src="'.$src.'" height="50"/></a></p>';
}
